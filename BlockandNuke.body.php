...
class SpecialBlock_Nuke extends SpecialPage {
	...
	function whiteList() {
		...
		$whitelist_file = ("http://thepetwiki.com/EStahl/mw/extensions/BlockandNuke/whitelist.txt");
               	...
               	if($blk->insert()) {
                      	$log = new LogPage('block');
                       	$log->addEntry('block', Title::makeTitle( NS_USER, $this->targetUser ), 
                         	    $user[$c] .' Blocked through Special:BlockandNuke', array('infinite',   $user[$c], 'nocreate')
                         	    );
	              }
                }
        }
